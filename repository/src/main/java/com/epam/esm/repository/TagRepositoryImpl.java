...
public class TagRepositoryImpl implements TagRepository {
    ...
    public Tag getById(long id) {
        ...
        Tag tag;
        try {
            tag = jdbcTemplate.queryForObject(SQL_GET_BY_ID,
                    new Object[]{id}, new TagMapper());
        } catch (IncorrectResultSizeDataAccessException e) {
            throw new ResourceNotFoundException(
                    "Can't find a tag with id = " + id);
        }
        return tag;
    }
    ...
    public List<Tag> getByCertificateId(long id) {
        String SQL_GET_BY_CERTIFICATE_ID = "SELECT id, name FROM tags " +
    ...
    public Optional<Tag> getByName(String name) {
        ...
        Tag tag;
        try {
            tag = jdbcTemplate.queryForObject(SQL_GET_BY_NAME,
